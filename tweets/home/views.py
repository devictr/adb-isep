...
@api_view(['POST'])
def get_day_tweets(request, ):
    data = json.loads(request.body)
    result = MONGO_COLLECTION.system_js.tweetsByDay(data["tv_show"], data["day"], data["month"], data["year"])
    return Response({"tweets_by_hour": result})


@api_view(['GET'])
def get_coords(request, tv_show_name):
    if tv_show_name == "All":
        result = MONGO_DATABASE.find({"coordinates": {"$ne": "null"}}, {"coordinates": 1, "_id": 0})
    else:
        result = MONGO_DATABASE.find({"coordinates": {"$ne": "null"}, "tv_show": tv_show_name}, {"coordinates": 1, "_id": 0})
    return Response({"coords": json.loads(json_util.dumps(result))})